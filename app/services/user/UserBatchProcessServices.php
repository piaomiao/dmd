	...
	public function batchProcess(int $type, array $uids, array $input_data, bool $is_all = false, array $where = [])
	{
		...
		switch ($type) {
			...
			case 6://上级推广人
                ...
                break;
			case 99://发送图文消息
				$new_id = $input_data['id'] ?? 0;
				if (!$new_id) throw new ValidateException('请选择要发送的图文消息');
				$wechatNewsCategoryServices = app()->make(WechatNewsCategoryServices::class);
				$list = $wechatNewsCategoryServices->getWechatNewsItem((int)$new_id);
				$wechatNews = [];
				if ($list) {
					...
					$wechatNews = $wechatNewsCategoryServices->wechatPush($wechatNews);
				}
				$data['wechat_news'] = $wechatNews;
