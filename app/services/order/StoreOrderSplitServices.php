...
//                $order_data['order_id'] = $storeOrderCreateServices->getNewOrderId();
        ...
        foreach ($cart_ids as $cart) {
			...
			$cart_data['write_times'] = bcmul((string)$cart['cart_num'], (string)(max($_info['productInfo']['attrInfo']['write_times'] ?? 1, 1)),0);
            ...
            if (!$i) {
                //无核销 ｜｜ 有核销剩余数量大于拆分数量
                $surplus_num = !$cart_data['writeoff_time'] || $cart_data['write_surplus_times'] >= $cart_data['write_times'] ? $cart_data['write_times'] : $cart_data['write_surplus_times'];
            ...
            if ($cart_data['write_surplus_times'] == $cart_data['write_times']) {//未核销
