...
class StoreOrderServices extends BaseServices
{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getOneOrderList(string $key, int $uid, array $with = [])
    {
        return $this->dao->getUserOrderDetail($key, $uid, array_merge(['user', 'spread', 'refund'], $with));
    }
...
//            'week_ratio' => $user_week_ratio,
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getOrderConfirmData(array $user, $cartId, bool $new, int $addressId, int $shipping_type = 1, int $store_id = 0, int $coupon_id = 0, int $luckRecordId = 0)
    {
        $addr = $data = [];
        $uid = (int)$user['uid'];
