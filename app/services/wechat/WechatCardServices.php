...
class WechatCardServices extends BaseServices
{
    ...
    public function createForm($formData)
    {
        ...
        $f[] = $this->builder->frameImage('logo_url', 'LOGO', $this->url(config('admin.admin_prefix') . '/widget.images/index', ['fodder' => 'logo_url'], true), $formData['logo_url'] ?? '')->info('建议300*300')->icon('ios-add')->width('960px')->height('550px')->modal(['footer-hide' => true]);
        $f[] = $this->builder->frameImage('background_pic_url', '背景图', $this->url(config('admin.admin_prefix') . '/widget.images/index', ['fodder' => 'background_pic_url'], true), $formData['background_pic_url'] ?? '')->info('建议1000*600')->icon('ios-add')->width('960px')->height('550px')->modal(['footer-hide' => true]);
