...
class VideoCommentServices extends BaseServices
{
    ...
    public function setReply(int $id, string $content)
    {
		...
		switch ($comment['type']) {
			case 0:
				...
				$avatar = sys_config('wap_login_logo');
				break;
			case 1://门店
				$store_id = (int)$comment['relation_id'];
				if ($store_id) {
					/** @var SystemStoreServices $storeServices */
					$storeServices = app()->make(SystemStoreServices::class);
					$store = $storeServices->get($store_id);
					$nickname = $store['name'];
					$avatar = $store['image'];
				}
				break;
	...
	* @return mixed
    ...
    {
        if ($video_id == 0) {
            $field[] = Form::frameImage('video', '视频', Url::buildUrl($store_id ? config('admin.store_prefix') . '/video.shortVideo/index' : config('admin.admin_prefix') . '/video.shortVideo/index', array('fodder' => 'video')))->icon('ios-add')->width('960px')->height('560px')->modal(['footer-hide' => true])->Props(['srcKey' => 'image']);
        } else {
            $field[] = Form::hidden('video_id', $video_id);
        }
        $field[] = Form::frameImage('avatar', '用户头像', Url::buildUrl($store_id ? config('admin.store_prefix') . '/widget.images/index' : config('admin.admin_prefix') . '/widget.images/index', array('fodder' => 'avatar')))->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true]);
        ...
        return create_form('添加自评', $field, Url::buildUrl('/marketing/video/comment/save_fictitious'), 'POST');
    }
