...
class StoreCouponIssueServices extends BaseServices
{
	...
	* @throws \think\db\exception\ModelNotFoundException
    ...
    public function getIssueCouponList(int $uid, array $where, string $field = '*', bool $is_product = false)
    {
        ...
        if ($where['product_id'] == 0) {
            ...
            $productInfo = $storeProductService->getOne(['id' => $where['product_id']], 'cate_id,brand_id');
			if ($productInfo) {
            ...
            if ($where['type'] == -1) { // PC端获取优惠券
				$list = $this->dao->getIssueCouponListNew($uid, ['product_id' => $where['product_id'], 'cate_id' => $cateId, 'brand_id' => $bandId], $field);
            } else {
				$coupon_where = ['type' => $where['type']];
                if ($where['type'] == 1) {
					$category_type['cate_id'] = $cateId;
                } elseif ($where['type'] == 2) {
					$category_type['product_id'] = $where['product_id'];
                } elseif ($where['type'] == 3) {
					$category_type['brand_id'] = $where['brand_id'];
				}
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function issueUserCoupon($id, $user, bool $more = true)
    {
	...
	* @throws \think\db\exception\ModelNotFoundException
    ...
    public function beUsableCouponList(int $uid, $cartId, bool $new, int $shipping_type = 1, int $store_id = 0)
    {
        ...
        $cartGroup = $services->getUserProductCartListV1($uid, $cartId, $new, [], $shipping_type, $store_id, 0, 0,true);
