...
class LiveAnchorServices extends BaseServices
{
    ...
    public function add(int $id)
    {
        ...
        if (!$anchor) {
            ...
            $field[] = Form::frameImage('cover_img', '主播图像', Url::buildUrl(config('admin.admin_prefix') .  '/widget.images/index', array('fodder' => 'cover_img')), '')->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true])->appendValidate(Iview::validateStr()->required()->message('请选择图像'));
        } else {
            $title = '修改主播';
            ...
            $field[] = Form::frameImage('cover_img', '主播图像', Url::buildUrl(config('admin.admin_prefix') .  '/widget.images/index', array('fodder' => 'cover_img')), $anchor->getData('cover_img'))->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true])->appendValidate(Iview::validateStr()->required()->message('请选择图像'));
        }
