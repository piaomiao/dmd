...
//                $valueNew[$count]['quota'] = $sukValue[$suk]['quota'] ? intval($sukValue[$suk]['quota']) : 0;
	...
	*/
    public function getBargain(int $uid, int $id)
    {
         ...
         * 判断配送方式
         */
        $storeInfo['delivery_type'] = $storeProductServices->getDeliveryType((int)$storeInfo['id'], (int)$storeInfo['type'], (int)$storeInfo['relation_id'], $storeInfo['delivery_type']);
    ...
    public function setBargain(int $uid, int $bargainId)
    {
        ...
        if ($count === false) {
			...
			[$bargainUserInfo, $price] = $this->transaction(function () use ($uid, $bargainId, $bargainInfo, $bargainUserService) {
				...
				$price = $bargainUserHelpService->setBargainUserHelp($bargainId, (int)$res['id'], $uid, $bargainInfo);
				$bargainUserInfo = $bargainUserService->get((int)$res['id']);
				return [$bargainUserInfo, $price];
			});
			$status = $this->getBargainStatus((int)$bargainUserInfo['id'], $bargainUserInfo);
			return compact('bargainUserInfo', 'price', 'status');
        }
    }
	 ...
	 * @throws \think\db\exception\ModelNotFoundException
	 */
	public function getBargainStatus(int $bargainUserTableId, $bargainUserInfo)
	{
		if (!$bargainUserInfo) {
			/** @var StoreBargainUserServices $bargainUserService */
			$bargainUserService = app()->make(StoreBargainUserServices::class);
		...
		if (bcsub($bargainUserInfo['bargain_price'], $bargainUserInfo['price'], 2) == $bargainUserInfo['bargain_price_min']) {
			$status = true;
		} else {
			$status = false;
		}
    ...
    public function setHelpBargain(int $uid, int $bargainId, int $bargainUserUid)
    {
		...
		$bargainUserInfo = $bargainUserService->get($bargainUserTableId);
        if (!$count) {
			throw new ValidateException('您已经帮砍过此砍价');
		}
		...
		$status = $this->getBargainStatus($bargainUserTableId, $bargainUserInfo);
        return compact('bargainUserInfo', 'price', 'status');
    }
