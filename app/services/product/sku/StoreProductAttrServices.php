...
//            sort($value['detail'], SORT_STRING);
            ...
            $valueGroup[$sku] = [
				...
				'write_times' => $value['write_times'] ?? 1,
				'write_valid' => $value['write_valid'] ?? 1,
				'write_days' => $value['write_days'] ?? $value['days'] ?? 0,
				'write_start' => ($value['section_time'][0] ?? '') ? strtotime($value['section_time'][0] ?? '') : 0,
				'write_end' => ($value['section_time'][1] ?? '') ?  strtotime($value['section_time'][1] ?? '') : 0,
            ];
        }
