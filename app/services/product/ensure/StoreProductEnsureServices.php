...
class StoreProductEnsureServices extends BaseServices
{
    ...
    public function form($info = [])
    {
        ...
        $f[] = Form::frameImage('image', '图标(建议尺寸：100px*100px)', Url::buildUrl(config('admin.admin_prefix') .  '/widget.images/index', array('fodder' => 'image')), $info['image'] ?? '')->icon('ios-add')->width('960px')->appendValidate(Iview::validateStr()->message('请选择图标(建议尺寸：100px*100px)')->required())->height('505px')->modal(['footer-hide' => true]);
