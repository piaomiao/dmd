        ...
        switch ($item['mark']) {
            ...
            case 'rebate_points_orders' : // 未支付积分订单退积分
                ...
                try {
                    ...
                    response_log_write([
                        'message' => '执行积分订单未支付时退积分任务失败:' . $e->getMessage(),
                        'file' => $e->getFile(),
                        'line' => $e->getLine()
                    ]);
                }
            ...
            case 'code_not_operating' : // 桌码长期未操作取消桌码记录
                /** @var UserCollageServices $service */
                $service = app()->make(UserCollageServices::class);
                try {
                    return $service->tableCodeNotOperating(10);
                } catch (\Throwable $e) {
                    response_log_write([
                        'message' => '执行桌码长期未操作取消桌码记录任务失败:' . $e->getMessage(),
                        'file' => $e->getFile(),
                        'line' => $e->getLine()
                    ]);
                }
            ...
            case 'collate_not_operating' : // 拼单长期未操作取消拼单记录
                ...
                try {
                    ...
                    response_log_write([
                        'message' => '执行拼单长期未操作取消拼单记录任务失败:' . $e->getMessage(),
                        'file' => $e->getFile(),
                        'line' => $e->getLine()
                    ]);
                }
